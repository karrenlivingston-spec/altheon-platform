...
export const CLINIC_DOC_CATEGORY_OPTIONS = [
...
function categoryLabel(value: string | null | undefined): string {
  const v = (value ?? "").trim();
  if (!v) return "—";
  const preset = CLINIC_DOC_CATEGORY_OPTIONS.find((o) => o.value === v);
...
export default function ClinicReferenceDocumentsPanel() {
  ...
  const [uploadCategory, setUploadCategory] = useState<string>(
    CLINIC_DOC_CATEGORY_OPTIONS[0].value,
  );
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          parseApiError(json, `Could not load research documents (${res.status}).`),
        );
      }
      ...
      setDocuments([]);
      setError(
        e instanceof Error ? e.message : "Could not load research documents.",
      );
    } finally {
      setLoading(false);
    }
  ...
  function resetUploadForm() {
    setUploadTitle("");
    setUploadCategory(CLINIC_DOC_CATEGORY_OPTIONS[0].value);
  ...
  async function deleteDocument(docId: string, title: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const json: unknown = await res.json().catch(() => ({}));
        throw new Error(parseApiError(json, `Delete failed (${res.status}).`));
      }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Research library</h2>
          <p className="mt-1 text-sm text-gray-600">
            Clinic reference documents — testing protocols, algorithms, and research
            materials.
          </p>
        </div>
        {isAdmin ? (
              ...
              {loading ? (
                ...
                documents.map((row) => (
                  <tr key={row.id} className={DS_TR}>
                    ...
                    <td className={DS_TD_PRIMARY}>{categoryLabel(row.category)}</td>
          ...
          >
            ...
            <label className="mb-4 block text-sm font-medium text-gray-700">
              ...
              <select
                ...
                className={`mt-1 block w-full ${DS_INPUT}`}
              >
                {CLINIC_DOC_CATEGORY_OPTIONS.map((o) => (
